...
系统 SHALL 计算各包管理器缓存目录的磁盘占用大小。缓存目录清单：apt 缓存（/var/cache/apt/archives/）、pip 缓存（~/.cache/pip/）、npm 缓存（~/.npm/）、conda 包缓存（~/anaconda3/pkgs/）、cargo 注册表缓存（~/.cargo/registry/）。Docker 占用通过 `docker system df --format='{{json .}}'` 获取（需先通过 `docker info` 验证 daemon 可用，serde_json 解析）。对于不存在的缓存目录，系统 SHALL 静默跳过。大小计算使用 Rust std::fs 递归遍历 + metadata().len() 求和（非 du 命令），单位为 bytes，UI 显示为 IEC 格式（KiB/MiB/GiB）。所有文件系统操作通过 tokio::fs 异步执行。
...
- **THEN** 通过 serde_json 解析 JSON 输出，分别显示 Images 和 Containers 的占用大小
...
系统 SHALL 生成可操作的清理建议列表，每条建议为 CleanupSuggestion { description, estimated_bytes, command, requires_sudo, risk_level: RiskLevel }。清理操作 SHALL 在用户点击"执行清理"按钮后弹出 adw::AlertDialog 确认对话框（显示将执行的命令、预估回收空间、是否需要 sudo），用户确认后才执行。需要 sudo 的操作通过 pkexec 提权（std::process::Command 同步执行，因为需要等待 polkit 交互）。每次清理操作独立确认，不提供会话级免确认。清理成功后自动重新扫描受影响的缓存项。

#### Scenario: Suggest apt cache cleanup
- **GIVEN** apt 缓存占用 1.2 GiB
- **WHEN** 系统生成清理建议
- **THEN** 建议列表包含 CleanupSuggestion { description: "清理 apt 下载缓存", estimated_bytes: 1288490188, command: "apt clean", requires_sudo: true, risk_level: RiskLevel::Safe }

#### Scenario: Suggest pip cache cleanup
- **GIVEN** ~/.cache/pip/ 占用 500 MiB
- **WHEN** 系统生成清理建议
- **THEN** 建议列表包含 CleanupSuggestion { description: "清理 pip 下载缓存", estimated_bytes: 524288000, command: "pip3 cache purge", requires_sudo: false, risk_level: RiskLevel::Safe }

#### Scenario: User confirms cleanup
- **GIVEN** 用户查看清理建议列表
- **WHEN** 用户点击"清理 apt 下载缓存"的"执行清理"按钮
- **THEN** 弹出 adw::AlertDialog 显示：命令 "apt clean"、预估回收 "1.2 GiB"、标注"需要管理员权限"，包含"取消"和"确认执行"按钮
...
- **WHEN** 清理命令返回非零退出码
- **THEN** 显示 adw::AlertDialog 错误提示"清理失败: <错误信息>"，缓存大小不变，"执行清理"按钮恢复可点击状态

#### Scenario: Button disabled during execution
- **GIVEN** 用户确认执行某条清理建议
- **WHEN** 清理命令正在执行中
- **THEN** 该建议的"执行清理"按钮显示为 adw::Spinner + "执行中..."，不可重复点击

### Requirement: Disk Analysis View Layout
系统 SHALL 在磁盘分析视图中展示三个区域（使用 gtk4::ScrolledWindow 垂直滚动）：顶部为缓存占用概览（每个缓存项使用 adw::ActionRow + gtk4::LevelBar 进度条显示占用大小），中部为最大包排行列表（gtk4::ListView），底部为清理建议列表（每条建议含描述、预估空间、"执行清理"按钮）。视图采用懒加载，进入时触发扫描，结果通过 glib::spawn_future_local 从 tokio 异步任务回传 GTK 主线程。
...
- **GIVEN** 用户首次进入磁盘分析视图
- **WHEN** 缓存适配器正在后台扫描
- **THEN** 视图显示 adw::Spinner 加载指示器，已完成的缓存项立即渲染
